...
public class Agent : MonoBehaviour
{
    ...
    public bool collided;//To tell if the agent has hit a wall

    void FixedUpdate()//FixedUpdate is called at a constant interval
    {
        if (!collided)//if the agent has not collided with the wall, it uses the neural network to get an output
        {
            ...
            transform.Rotate(0, output[0] * rotation, 0, Space.World);//controls the agents movement
            transform.position += this.transform.right * output[1] * speed;//controls the agents turning
        }
    }
    ...
    {
        if(collision.collider.gameObject.layer == LayerMask.NameToLayer("CheckPoint"))//check if the agent passes a checkpoint
        {
            ...
            for (int i=0; i < checkPoints.Length; i++)
            {
                ...
                {
                    position++;//if the checkpoint is one ahead of it, it increments the position, which is used for the fitness/performance of the network
                    break;
                }
            }
        }
        ...
        {
            collided = true;//stop operation if agent has collided
        }
    }
